class EnhancedCalculatorApp {
  ...
  initializeElements() {
    ...
    console.log("Elements initialized:", {
      "mathInput": !!this.elements.mathInput,
      "equationHidden": !!this.elements.equationHidden,
      "gFunctionInput": !!this.elements.gFunctionInput,
      "gFunctionHidden": !!this.elements.gFunctionHidden
    });
  ...
  initializeAllMathFields() {
    ...
    // Check for dynamic equation fields
    this.initializeDynamicEquationFields();
    
    // Dispatch custom event for other components to know mathfields are ready
    document.dispatchEvent(new CustomEvent('mathFieldsUpdated'));
  }
  ...
  handleKeyPress(key) {
    ...
    console.log("Current active field:", this.activeMathField);
    if (this.elements.gFunctionHidden) {
      console.log("Current gFunctionHidden value:", this.elements.gFunctionHidden.value);
    }
    ...
    // If no active field, try to find one with focus
    if (!currentField) {
      const focusedField = document.activeElement;
      if (focusedField) {
        if (this.INPUT_SELECTORS.some(selector => focusedField.matches && focusedField.matches(selector))) {
          if (focusedField.mathquillInstance) {
            currentField = focusedField.mathquillInstance;
          } else if (focusedField.id === 'math-input' || focusedField.id === 'gFunctionInput') {
            currentField = this.allMathFields.get(focusedField.id);
          } else if (focusedField.id.startsWith('mathquill_equation_')) {
            const equationNumber = focusedField.id.split('_').pop();
            currentField = this.allMathFields.get(`equation_${equationNumber}`);
          } else {
    ...
    // If still no field, use the first available
    if (!currentField) {
      const firstMathField = document.querySelector(this.MATHQUILL_ELIGIBLE_SELECTORS.join(', '));
      if (firstMathField) {
        if (firstMathField.mathquillInstance) {
          currentField = firstMathField.mathquillInstance;
        } else if (firstMathField.id === 'math-input' || firstMathField.id === 'gFunctionInput') {
          currentField = this.allMathFields.get(firstMathField.id);
        }
      }
    ...
    // If it's a MathQuill field, insert symbol; otherwise, write to the input
  ...
  insertSymbol(currentMathField, key) {
    ...
    const actions = {
      ...
      "**-1": () => {
        console.log("Action: inverse");
        currentMathField.write("^{-1}");
      },
      "e": () => {
        console.log("Action: e");
        currentMathField.write("e");
      }
    };
    ...
    // Re-focus the MathQuill field
    currentMathField.focus();
    console.log("MathQuill field re-focused");
    
    // Force update of hidden fields after symbol insertion
    if (currentMathField === this.allMathFields.get('math-input') && this.elements.equationHidden) {
      const latex = currentMathField.latex();
      this.elements.equationHidden.value = this.latexToJavaScript(latex);
    } else if (currentMathField === this.allMathFields.get('gFunctionInput') && this.elements.gFunctionHidden) {
      const latex = currentMathField.latex();
      const parsedExpression = this.latexToJavaScript(latex);
      this.elements.gFunctionHidden.value = parsedExpression;
      console.log(`Force updated gFunctionHidden after key press: ${parsedExpression}`);
    }
  }
  ...
  setupEventListeners() {
    ...
    // Listen for custom event that signals mathFields need updating
    document.addEventListener('mathFieldsUpdated', () => {
      this.initializeAllMathFields();
    });
    
    // Add form submit event listener to ensure all hidden fields are updated
    const form = document.getElementById('calculator-form');
    if (form) {
      form.addEventListener('submit', (e) => {
        // Update all hidden fields one last time before submission
        if (this.allMathFields.get('math-input') && this.elements.equationHidden) {
          const latex = this.allMathFields.get('math-input').latex();
          this.elements.equationHidden.value = this.latexToJavaScript(latex);
        }
        
        if (this.allMathFields.get('gFunctionInput') && this.elements.gFunctionHidden) {
          const latex = this.allMathFields.get('gFunctionInput').latex();
          const parsedExpression = this.latexToJavaScript(latex);
          this.elements.gFunctionHidden.value = parsedExpression;
          console.log(`Updated gFunctionHidden before form submission: ${parsedExpression}`);
        }
      });
    }
  }
  ...
  latexToJavaScript(latex) {
    // For g(x) field, specifically handle the conversion more carefully
    let result = latex
    ...
    // Convert implicit multiplication (2x to 2*x)
    result = result.replace(/(\d)([a-zA-Z])/g, "$1*$2");
    
    // Convert caret exponentiation to JavaScript's ** operator
    result = result.replace(/\^{([^{}]+)}/g, "**($1)").replace(/\^(\d+)/g, "**$1");
